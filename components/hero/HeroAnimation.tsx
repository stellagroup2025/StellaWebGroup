...
export function HeroAnimation() {
    ...
    useEffect(() => {
        ...
        const CONFIG = {
            pointCount: 36000,
            gridSize: 5000, // Reduced slightly for density
        ...
        void main() {
            ...
            // Gentle sine waves that make the grid 'breath' even without interaction
    ...
    `
        ...
        void main() {
            ...
            // CINEMATIC BOKEH EFFECT (TUNED FOR INTENSITY)
            float mixRatio = smoothstep(-50.0, 200.0, vHeight);
            
            // 0.45 = Sharp | 0.2 = Soft but with SOLID CORE (Fixed ghosting issue)
            float edgeSoftness = mix(0.45, 0.2, mixRatio);
            float alpha = 1.0 - smoothstep(edgeSoftness, 0.5, d);

            // COLOR DEPTH - ULTRA VIVID
            // On white background, we need DARKER/PURIER blues to look intense.
            // Lighter cyan vanishes. Pure Blue pops.
            
            vec3 deepColor = vec3(0.05, 0.2, 0.9); // Base Royal
            vec3 brightColor = vec3(0.0, 0.4, 1.0); // Electric Blue
            vec3 highlightColor = vec3(0.0, 0.0, 1.0); // Pure Deep Blue (Maximum Contrast)
            
            vec3 finalColor = mix(deepColor, brightColor, mixRatio);
            finalColor = mix(finalColor, highlightColor, smoothstep(0.8, 1.0, mixRatio));

            // Boost alpha slightly on peaks to ensure visibility
            float alphaBoost = 1.0 + (mixRatio * 0.5);
            gl_FragColor = vec4( finalColor, opacity * alpha * alphaBoost );
        }
    `

        const material = new THREE.ShaderMaterial({
            uniforms: {
                color: { value: new THREE.Color(CONFIG.color) }, // Kept for reference
            ...
            for (let i = 0; i < positions.length; i += 3) {
                ...
                if (dist < effectiveRadius) {
                    ...
                    if (speed > 0.1) {
                        ...
                        // 1. Linear Drag (The "Jelly" effect - Stretch)
                        const deform = alignment * clampedSpeed * 12 * Math.pow(influence, 2)
                        px += jellyAxis.x * deform
                        py += jellyAxis.y * deform

                        // 2. VORTEX TURBULENCE (The "Swirl" effect) - Optimized
                        // Instead of running fluid sims, we just apply a tangential force.
                        // Particles rotate slightly around the cursor movement trail.

                        const swirlStrength = clampedSpeed * 15.0 * Math.pow(influence, 3)

                        // Deterministic "Direction" based on position to create chaotic micro-eddies
                        // Some spin left, some spin right based on their grid index
                        const spinDir = (i % 2 === 0) ? 1 : -1

                        // Apply Perpendicular Force (-y, x)
                        px += -dirY * swirlStrength * spinDir
                        py += dirX * swirlStrength * spinDir
                    }
